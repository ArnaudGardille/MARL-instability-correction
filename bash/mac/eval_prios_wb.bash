...
for ((run=1; run<3; run+=1)); do
  for ((i=0; i<2; i+=1)); do
    for ((j=1; j<2; j+=1)); do
      /Users/gardille/opt/anaconda3/envs/torch_rl/bin/python /Users/gardille/development/Water-Bomber-Env/src/iql_gym.py  --env-id water-bomber  --enforce-coop --run-name ${names[i]}-${lr[j]}-$run ${configs[i]} --fixed-buffer --load-buffer-from /Users/gardille/development/Water-Bomber-Env/results/water-bomber-buffer --buffer-size 2000000 --train-frequency 1 --total-timesteps 10000 --learning-starts 0 --learning-rate ${lr[j]} --device cpu --seed $run 
    done
  done
done
