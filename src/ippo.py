...
class QAgent():
    ...
    def visualize_q_values(self, env, global_step):
        ...
        observation = observation[self.id]['observation']
        observation[-1] = 5
        ...
        for x in range(env.X_MAX+1):
            for y in range(env.Y_MAX+1):
                observation[4+2*self.id] = x
                observation[4+2*self.id + 1] = y
                #print(observation)

                action_mask= env.get_action_mask(x,y)
                observation = observation.to(device)
                #print(self.q_network(observation).detach().cpu()*action_mask)
                target_max = (self.q_network(observation).detach().cpu()*action_mask).max()
